...
export const Dashboard: React.FC = () => {
  ...
  const loadData = async () => {
    ...
    try {
      const [scriptsRes, runsRes, healingRes] = await Promise.all([
        ...
        axios
          .get(`${API_URL}/self-healing/suggestions`, { headers })
      ...
      const healingData = (healingRes as any).data?.suggestions || [];
      ...
      setStats({
        ...
        pendingHealing: healingData.filter((s: HealingSuggestion) => s.status === 'pending').length
      });
    } catch (error: any) {
  ...
  const analyzeLocatorWithAI = async () => {
    if (!testLocator) {
      alert('Please enter a locator to analyze');
      return;
    }
    ...
    try {
      let elementSnapshot = {};
      if (testElementSnapshot.trim()) {
        try {
          elementSnapshot = JSON.parse(testElementSnapshot);
        } catch (e) {
          alert('Invalid JSON in element snapshot');
      ...
      console.error('Error analyzing locator:', error);
      alert('Failed to analyze locator: ' + (error.response?.data?.error || error.message));
    } finally {
      setAiAnalyzing(false);
    }
  };
  ...
  const autoHealSuggestion = async (suggestionId: number) => {
    try {
      ...
      console.error('Error auto-healing:', error);
      alert('Auto-heal failed: ' + (error.response?.data?.message || error.message));
    }
  };
  ...
  const menuItems = [
    ...
    { id: 'healing', icon: '💊', label: 'Self-Healing', category: 'Test Management', badge: stats.pendingHealing },
    { id: 'aihealing', icon: '🤖', label: 'AI Healing', category: 'Test Management' },
    { id: 'testdata', icon: '🗄️', label: 'Test Data', category: 'Data Management' },
    { id: 'apitesting', icon: '🔌', label: 'API Testing', category: 'Testing Tools' },
    { id: 'allure', icon: '📈', label: 'Allure Reports', category: 'Reports' },
    ...
    <div className="dashboard-container">
      ...
      <main className="dashboard-main">
        <div className="dashboard-content">
          ...
          {activeView === 'healing' && (
            <div className="view-container">
              <h1 className="view-title">Self-Healing Suggestions</h1>
              <div className="healing-stats">
                <div className="healing-stat">
                  <span className="stat-number">{healingSuggestions.length}</span>
                  <span className="stat-text">Total</span>
                </div>
                <div className="healing-stat pending">
                  <span className="stat-number">{stats.pendingHealing}</span>
                  <span className="stat-text">Pending</span>
                </div>
                <div className="healing-stat approved">
                  <span className="stat-number">
                    {healingSuggestions.filter(s => s.status === 'approved').length}
                  </span>
              ...
              {healingSuggestions.length === 0 ? (
                <div className="empty-state">
                  <div className="empty-icon">💊</div>
                  <h3>No Self-Healing Suggestions</h3>
                  <p>Suggestions will appear when tests fail with broken locators.</p>
                  <button className="btn-primary" onClick={() => setActiveView('aihealing')}>
                    🤖 Try AI Healing
                  </button>
                </div>
              ) : (
                <div className="healing-list">
                  {healingSuggestions.filter(s => s.status === 'pending').map((suggestion) => {
                    const isHighConfidence = suggestion.confidence >= 0.85;
                    return (
                      <div key={suggestion.id} className="healing-card">
                        <div className="healing-locators">
                          <div className="locator-section broken">
                            <div className="locator-label">❌ Broken</div>
                            <code className="locator-code">{suggestion.brokenLocator}</code>
                          </div>
                          <div className="locator-arrow">→</div>
                          <div className="locator-section suggested">
                            <div className="locator-label">✅ Suggested</div>
                            <code className="locator-code">{suggestion.validLocator}</code>
                          </div>
                        </div>
                        <div className="healing-footer">
                          <div className="healing-meta">
                            <span className="confidence" style={{
                              backgroundColor: isHighConfidence ? '#d1fae5' : '#fef3c7',
                              ...
                              {isHighConfidence && ' (Auto-Healable)'}
                            </span>
                            {suggestion.scriptName && <span>📝 {suggestion.scriptName}</span>}
                          ...
                          <div className="healing-actions">
                            {isHighConfidence && (
                              <button 
                                className="btn-primary" 
                                onClick={() => autoHealSuggestion(parseInt(suggestion.id))}
                                style={{ marginRight: '8px' }}
                              >
                                🤖 AI Auto-Heal
                              </button>
                            )}
                            <button className="btn-approve" onClick={() => approveSuggestion(suggestion.id)}>
                              ✓ Approve
                            </button>
                            <button className="btn-reject" onClick={() => rejectSuggestion(suggestion.id)}>
                              ✗ Reject
                            </button>
                          </div>
                        </div>
    ...
    "aria-label": "Submit form"
          ...
          {activeView === 'allure' && (
            <div className="view-container full-height">
              <h1 className="view-title">Allure Test Reports</h1>
              {selectedReport ? (
                ...
                <div className="empty-state">
                  ...
                  <p>Generate or view an Allure report from the Test Runs section.</p>
